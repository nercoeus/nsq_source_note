...
type TopicStats struct {
	TopicName    string         `json:"topic_name"`    // topic 名称
	Channels     []ChannelStats `json:"channels"`      // topic 中的 channel 的状态
	Depth        int64          `json:"depth"`         // topic 中消息的长度
	BackendDepth int64          `json:"backend_depth"` // 磁盘中消息的长度
	MessageCount uint64         `json:"message_count"` // 所有 message 长度
	MessageBytes uint64         `json:"message_bytes"` // 所有 message 字节数
	Paused       bool           `json:"paused"`        // topic 是否暂停

	E2eProcessingLatency *quantile.Result `json:"e2e_processing_latency"`  // 性能相关参数
}
...
type ChannelStats struct {
	ChannelName   string        `json:"channel_name"`    // channel 名称
	Depth         int64         `json:"depth"`           // channel 中消息数
	BackendDepth  int64         `json:"backend_depth"`   // channel 中磁盘消息数
	InFlightCount int           `json:"in_flight_count"` // 处在 InFlight 中的消息数
	DeferredCount int           `json:"deferred_count"`  // 处在延迟队列中的消息数
	MessageCount  uint64        `json:"message_count"`   // channel 发送消息数
	RequeueCount  uint64        `json:"requeue_count"`   // 请求数量
	TimeoutCount  uint64        `json:"timeout_count"`   // 超时消息数
	ClientCount   int           `json:"client_count"`    // 客户端数
	Clients       []ClientStats `json:"clients"`         // 所有客户端信息
